...
interface AddBrandModalProps {
  onClose: () => void;
  onSuccess: (form: any, file?: File) => void;
}
...
export default function AddBrandModal({ onClose, onSuccess }: AddBrandModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await onSuccess({ name, slug }, file || undefined);
      onClose();
    } catch (err: any) {
      toast.error(err?.message || 'Thêm thương hiệu thất bại');
    } finally {
      setLoading(false);
    }
  };
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-[50%] left-[50%] w-[420px] -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl p-6 shadow-2xl z-50">
          <div className="flex justify-between items-center mb-4">
            <Dialog.Title className="text-xl font-semibold text-gray-800">Thêm thương hiệu</Dialog.Title>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col">
              <label className="text-sm font-medium text-gray-700 mb-1">Slug (tùy chọn)</label>
              <input
                type="text"
                value={slug}
